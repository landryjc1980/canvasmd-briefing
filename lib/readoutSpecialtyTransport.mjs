...
const READ_RPCS = new Set(["x_posts_needing_features", "article_attention", "article_attention_people"]);
function recapObservation(body, response, text) {
  ...
  const complete = (!needsMain || hasText(output?.headline)) && missingStoryIds.length === 0;
...
export function createSpecialtyTransport({ baseUrl, signal, fetchImpl = fetch, record, allowCapturedRecap = false, maxCacheBytes = 96 * 1024 * 1024 }) {
  const origin = new URL(baseUrl).origin, cache = new Map(), inFlight = new Map();
  const failures = new Map();
  const recaps = new Map();
  let bytes = 0, requests = 0, cacheHits = 0, coalescedRequests = 0, transientFailures = 0;
  const perform = async (input, init = {}) => {
    ...
    if (cacheable && cache.has(key)) { cacheHits++; const stored = cache.get(key); return new Response(stored.status === 204 ? null : stored.body, stored); }
    const dedupe = cacheable || recap;
    if (dedupe && inFlight.has(key)) {
      coalescedRequests++;
      const stored = await inFlight.get(key);
      return new Response(stored.status === 204 ? null : stored.body, stored);
    }
    const request = (async () => {
      requests++;
  ...
  return {
    fetch: perform,
    stats: () => ({ requests, cacheHits, coalescedRequests, cacheBytes: bytes, transientFailures, failures: [...failures.values()], recaps: [...recaps.values()] }),
    assertHealthy: () => {
