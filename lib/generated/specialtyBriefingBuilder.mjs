...
var REMOVABLE_QUERY_PARAM = /^(?:utm_.+|mtm_.+|at_(?:medium|campaign|custom\d+)|fbclid|gclid|dclid|msclkid|mc_.+|mkt_tok|dgcid|hss_channel|creator|team|guestaccesskey|login|redirectedfrom|st)$/i;
...
// supabase/functions/_shared/readoutArticleIdentity.ts
function exactBibliographicArticleGroup(members, requested) {
  if (!members.length) return false;
  if (requested && !members.every((member) => paperIdentifiersCompatible(requested, member))) return false;
  if (!members.every((member, index) => members.slice(index + 1).every((other) => paperIdentifiersCompatible(member, other)))) return false;
  return members.some((anchor) => (!requested || !(requested.article_doi || requested.pmid) || bibliographicIdentitiesMatch(requested, anchor)) && members.every((member) => bibliographicIdentitiesMatch(anchor, member)));
}

// supabase/functions/_shared/articleAttention.ts
var BATCH_SIZE = 100;
var cleanId = (value) => typeof value === "string" && value.trim() ? value.trim() : null;
var asRows = (value) => Array.isArray(value) ? value : [];
async function readArticleAttention(db, articleIds, startAt, endAt) {
  const requested = [...new Set(articleIds.map(cleanId).filter((id) => !!id))];
  const output = /* @__PURE__ */ new Map();
  for (let offset = 0; offset < requested.length; offset += BATCH_SIZE) {
    const { data, error } = await db.rpc("article_attention", {
      p_article_ids: requested.slice(offset, offset + BATCH_SIZE),
      p_start_at: startAt,
      p_end_at: endAt,
      p_metric: "clinician_shares"
    });
    if (error) throw new Error(`article_attention: ${error.message ?? "unknown error"}`);
    for (const row of asRows(data)) {
      const articleId = cleanId(row.article_id);
      if (articleId) output.set(articleId, row);
    }
  }
  return output;
}
async function readArticleAttentionPeople(db, articleId, startAt, endAt, limit = 12) {
  const { data, error } = await db.rpc("article_attention_people", {
  ...
  if (error) throw new Error(`article_attention_people: ${error.message ?? "unknown error"}`);
  const rows = asRows(data);
  const summary = rows.find((row) => row.row_kind === "summary");
  const availability = String(summary?.availability ?? "unavailable_response");
  const count = availability === "available" && Number.isInteger(summary?.total_people) ? Number(summary?.total_people) : null;
  const people = rows.flatMap((row) => {
    if (row.row_kind !== "person") return [];
    const personId = cleanId(row.person_id), sourceId = cleanId(row.x_source_id);
    if (!personId || !sourceId) return [];
    return [{
      personId,
      sourceId,
      name: String(row.display_name ?? "Clinician").trim() || "Clinician",
      handle: cleanId(row.x_handle),
      avatar: cleanId(row.avatar_url),
      firstMentionAt: cleanId(row.first_mention_at),
      lastMentionAt: cleanId(row.last_mention_at)
    }];
  });
  return { articleId: cleanId(summary?.article_id) ?? articleId, count, availability, people };
}
function unionEligibleArticleAttention(articleId, receipts, profiles, limit = 12) {
  const byIdentity = /* @__PURE__ */ new Map();
  for (const receipt of receipts) {
    const identity = cleanId(receipt.identityKey), personId = cleanId(receipt.personId), sourceId = cleanId(receipt.sourceId);
    if (!identity || !personId || !sourceId) continue;
    const prior = byIdentity.get(identity);
    const at = cleanId(receipt.postedAt);
    if (!prior) {
      byIdentity.set(identity, { receipt, first: at, last: at });
      continue;
    }
    if (at && (!prior.first || at < prior.first)) prior.first = at;
    if (at && (!prior.last || at > prior.last)) {
      prior.last = at;
      prior.receipt = receipt;
    }
  }
  const people = [...byIdentity.entries()].sort(([leftKey, left], [rightKey, right]) => String(right.last ?? "").localeCompare(String(left.last ?? "")) || leftKey.localeCompare(rightKey)).slice(0, Math.min(Math.max(Math.floor(limit), 1), 100)).flatMap(([, value]) => {
    const personId = cleanId(value.receipt.personId), sourceId = cleanId(value.receipt.sourceId);
    if (!personId || !sourceId) return [];
    const profile = profiles.get(personId) ?? { name: null, handle: null, avatar: null };
    return [{
      personId,
      sourceId,
      name: String(profile.name ?? "Clinician").trim() || "Clinician",
      handle: cleanId(profile.handle),
      avatar: cleanId(profile.avatar),
      firstMentionAt: value.first,
      lastMentionAt: value.last
    }];
  });
  return { articleId, count: byIdentity.size, availability: "available", people };
}
async function mapBounded(values, limit, map) {
  const results = new Array(values.length);
  let next = 0;
  const workers = Array.from({ length: Math.min(Math.max(1, Math.floor(limit)), values.length) }, async () => {
  ...
  return async function recapFor({ area, movers, events, stories = [], leadStories = [] }) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (!j?.headline && attempt === 0) {
          console.error(`briefing-recap ${area}: empty prose (attempt 1/2), retrying`);
          continue;
        }
        return { recap: null, whys: j?.whys ?? {}, headline: j?.headline ?? null, headlineStoryId: j?.headlineStoryId ?? null, moverHeadlines: j?.moverHeadlines ?? {}, storyWhys: j?.storyWhys ?? {}, storyHeadlines: j?.storyHeadlines ?? {} };
      } catch (e) {
...
function createSpecialtyBriefingBuilder(input) {
  ...
  async function buildArea(area, opts, effects) {
    ...
    const hydratedArticlesById = /* @__PURE__ */ new Map();
    const hydrateMentionEvidence = async (mentions) => {
      const postIds = [...new Set(mentions.map((row) => String(row.x_post_id ?? "")).filter(Boolean))];
      const sourceIds = [...new Set(mentions.map((row) => String(row.x_source_id ?? "")).filter(Boolean))];
      const articleIds = [...new Set(mentions.map((row) => String(row.article_id ?? "")).filter(Boolean))];
      const [posts, sources, articles] = await Promise.all([
        collectInIdBatches(postIds, (ids) => rows(sb.from("x_posts").select("id,x_post_id,content,lang,content_en,thread_parts,quoted_url,quoted_title,quoted_text,quoted_description,posted_at,metrics,metrics_24h,rt_tweet_id,rt_author_handle").in("id", ids))),
        collectInIdBatches(sourceIds, (ids) => rows(sb.from("x_sources").select("id,name,x_handle,avatar_url,source_type,active,entity_id,primary_institution").in("id", ids))),
        collectInIdBatches(articleIds, (ids) => rows(sb.from("x_shared_articles").select("id,title,canonical_url,journal,domain,article_doi,pmid,validated_publisher_url,publisher_url_status,publisher_url_source,pub_types,tumor_categories,abstract,description,pub_date,first_shared_at,onc_relevant,promotional").in("id", ids)))
      ]);
      const postsById = new Map(posts.map((row) => [String(row.id), row]));
      const sourcesById = new Map(sources.map((row) => [String(row.id), row]));
      const articlesById = new Map(articles.map((row) => [String(row.id), row]));
      for (const article of articles) hydratedArticlesById.set(String(article.id), article);
      return mentions.flatMap((mention) => {
        const x_posts = postsById.get(String(mention.x_post_id ?? ""));
        const x_sources = sourcesById.get(String(mention.x_source_id ?? ""));
        const x_shared_articles = articlesById.get(String(mention.article_id ?? ""));
        return x_posts && x_sources && x_shared_articles ? [{ ...mention, x_posts, x_sources, x_shared_articles }] : [];
      });
    };
    ...
    const mergeArticleAgg = (target, source) => {
      ...
      for (const articleId of source.canonicalArticleIds) target.canonicalArticleIds.add(articleId);
      target.pubDate ??= source.pubDate;
      target.firstSharedAt ??= source.firstSharedAt;
    ...
    await pageEachBatch((from, to) => sb.from("v_article_mentions").select("mention_id,article_id,x_post_id,x_source_id,posted_at,source_type,post_kind").eq("source_type", "kol").eq("is_share_kind", true).gte("posted_at", recentCutoff).order("mention_id", { ascending: true }).range(from, to), async (batch) => {
      const evidenceRows = await hydrateMentionEvidence(batch);
      await hydrateEvidenceOrigins(dropBanned(evidenceRows), sourceByHandle);
      for (const r of evidenceRows) {
        ...
        if (handle && isCredibleClinician(r.x_sources, clinicianSourceIds) && readHere) {
          ...
          if (!agg) {
            agg = { canonicalArticleIds: /* @__PURE__ */ new Set([String(r.article_id)]), title: candidateTitle, url: a.canonical_url, journal: a.journal ?? null, domain: a.domain ?? null, doi: a.article_doi ?? null, pmid: a.pmid ?? null, validatedPublisherUrl: a.validated_publisher_url ?? null, publisherUrlStatus: a.publisher_url_status ?? null, publisherUrlSource: a.publisher_url_source ?? null, pubTypes: a.pub_types ?? null, abstract: cleanArticleContext(a.abstract), description: cleanArticleContext(a.description), pubDate: a.pub_date ?? null, firstSharedAt: a.first_shared_at ?? null, areas, sharers: /* @__PURE__ */ new Map(), personSharers: /* @__PURE__ */ new Map(), posts: /* @__PURE__ */ new Map(), topLikes: 0, peerReviewed: isPeerReviewed(a) };
          ...
          agg.canonicalArticleIds.add(String(r.article_id));
    ...
    for (const [k, agg] of articleAgg) {
      readingByKey.set(k, {
        canonicalArticleIds: new Set(agg.canonicalArticleIds),
        title: agg.title,
        url: agg.url,
        journal: agg.journal,
        domain: agg.domain,
    ...
    await pageEachBatch((from, to) => sb.from("v_article_mentions").select("mention_id,article_id,x_post_id,x_source_id,posted_at,source_type,post_kind").in("source_type", ["organization", "journal", "news", "brand", "advocacy"]).eq("is_share_kind", true).gte("posted_at", recentCutoff).order("mention_id", { ascending: true }).range(from, to), async (batch) => {
      const evidenceRows = await hydrateMentionEvidence(batch);
      await hydrateEvidenceOrigins(dropBanned(evidenceRows), sourceByHandle);
      for (const r of evidenceRows) {
        const a = r.x_shared_articles;
        ...
        if (!rd) {
          rd = { canonicalArticleIds: /* @__PURE__ */ new Set([String(r.article_id)]), title: cleanTitle(a.title), url: a.canonical_url, journal: a.journal ?? null, domain: a.domain ?? null, doi: a.article_doi ?? null, pmid: a.pmid ?? null, validatedPublisherUrl: a.validated_publisher_url ?? null, publisherUrlStatus: a.publisher_url_status ?? null, publisherUrlSource: a.publisher_url_source ?? null, pubTypes: a.pub_types ?? null, abstract: cleanArticleContext(a.abstract), description: cleanArticleContext(a.description), pubDate: a.pub_date ?? null, firstSharedAt: a.first_shared_at ?? null, personSharers: /* @__PURE__ */ new Map(), publishers: /* @__PURE__ */ new Map(), evidence: /* @__PURE__ */ new Map(), topLikes: 0, peerReviewed: isPeerReviewed(a) };
        ...
        rd.canonicalArticleIds.add(String(r.article_id));
    ...
    for (const rd of readingByKey.values()) {
      ...
      for (const articleId of rd.canonicalArticleIds) ex.canonicalArticleIds.add(articleId);
    ...
    const readings = [...readingByTitle.values()].flat();
    const attentionEndAt = new Date(now).toISOString();
    const attentionCounts = await readArticleAttention(
      sb,
      readings.flatMap((reading) => [...reading.canonicalArticleIds]),
      recentCutoff,
      attentionEndAt
    );
    const exactGroupAttention = async (reading) => {
      const ids = [...reading.canonicalArticleIds].sort();
      if (ids.length < 2) return null;
      const members = ids.map((id) => hydratedArticlesById.get(id)).filter((article) => !!article);
      if (members.length !== ids.length || !exactBibliographicArticleGroup(members)) return null;
      const mentionRows = await pageAll((from, to) => sb.from("v_eligible_article_mentions").select("mention_id,identity_key,person_id,x_source_id,posted_at").in("article_id", ids).eq("counts_as_share", true).gte("posted_at", recentCutoff).lte("posted_at", attentionEndAt).order("posted_at", { ascending: false }).order("mention_id", { ascending: true }).range(from, to));
      const peopleIds = [...new Set(mentionRows.map((row) => String(row.person_id ?? "")).filter(Boolean))];
      const sourceIds = [...new Set(mentionRows.map((row) => String(row.x_source_id ?? "")).filter(Boolean))];
      const [people, sources] = await Promise.all([
        collectInIdBatches(peopleIds, (batch) => rows(sb.from("people").select("id,full_name").in("id", batch))),
        collectInIdBatches(sourceIds, (batch) => rows(sb.from("x_sources").select("id,x_handle,avatar_url").in("id", batch)))
      ]);
      const personById = new Map(people.map((person) => [String(person.id), person]));
      const sourceById = new Map(sources.map((source) => [String(source.id), source]));
      const profiles = /* @__PURE__ */ new Map();
      for (const row of mentionRows) {
        const personId = String(row.person_id ?? "").trim(), sourceId = String(row.x_source_id ?? "").trim();
        if (!personId || !sourceId) continue;
        const person = personById.get(personId), source = sourceById.get(sourceId);
        profiles.set(personId, { name: person?.full_name ?? null, handle: source?.x_handle ?? null, avatar: source?.avatar_url ?? null });
      }
      return unionEligibleArticleAttention(ids[0], mentionRows.map((row) => ({
        identityKey: row.identity_key ?? null,
        personId: row.person_id ?? null,
        sourceId: row.x_source_id ?? null,
        postedAt: row.posted_at ?? null
      })), profiles, 12);
    };
    const exactAttentionByReading = /* @__PURE__ */ new Map();
    const multiKeeperReadings = readings.filter((reading) => reading.canonicalArticleIds.size > 1);
    for (const [reading, attention] of await mapBounded(multiKeeperReadings, 4, async (reading2) => [reading2, await exactGroupAttention(reading2)])) {
      if (attention) exactAttentionByReading.set(reading, attention);
    }
    const canonicalArticleId = (reading) => {
      const candidates = [...reading.canonicalArticleIds].flatMap((id) => {
        const article = hydratedArticlesById.get(id);
        if (!article) return [];
        const exactIdentifier = bibliographicIdentitiesMatch(article, { article_doi: reading.doi, pmid: reading.pmid });
        const canonicalMatch = article.canonical_url === reading.url;
        const publisherMatch = !!reading.validatedPublisherUrl && article.validated_publisher_url === reading.validatedPublisherUrl;
        const score = (exactIdentifier ? 4 : 0) + (canonicalMatch ? 2 : 0) + (publisherMatch ? 1 : 0);
        return score ? [{ id, score }] : [];
      }).sort((left, right) => right.score - left.score || left.id.localeCompare(right.id));
      return candidates[0]?.id ?? null;
    };
    const clinicianCount = (reading) => {
      const exactAttention = exactAttentionByReading.get(reading);
      if (exactAttention) return exactAttention.count;
      const articleId = canonicalArticleId(reading);
      if (!articleId) return null;
      const row = attentionCounts.get(articleId);
      if (!row || row.availability !== "available" || !Number.isInteger(row.clinician_count)) {
        throw new Error(`article attention unavailable for ${articleId}`);
      }
      return Number(row.clinician_count);
    };
    const readingByReaderUrl = /* @__PURE__ */ new Map();
    const readerReadyReadings = [...readingByTitle.values()].flat().filter((a) => isReaderReadySourceTitle(a.title) && a.peerReviewed && !!readerArticleUrl({ ...a, validated_publisher_url: a.validatedPublisherUrl, publisher_url_status: a.publisherUrlStatus, publisher_url_source: a.publisherUrlSource }));
    const topArticles = readerReadyReadings.filter((a) => {
      const count = clinicianCount(a);
      return count != null && (count >= 1 || independentPubs(a) >= 2);
    }).map((a) => {
      ...
      readingByReaderUrl.set(readerUrl, a);
      return {
        title: a.title,
        url: readerUrl,
        journal: a.journal,
        ...
        sharers: clinicianCount(a) + a.publishers.size,
        kolSharers: clinicianCount(a),
    ...
    const resolvedTopAttention = await mapBounded(topArticles, 4, async (article) => {
      const reading = readingByReaderUrl.get(article.url);
      if (!reading) throw new Error(`missing reading source for ${article.url}`);
      const exactAttention = exactAttentionByReading.get(reading);
      if (exactAttention) return { article, attention: exactAttention };
      const articleId = canonicalArticleId(reading);
      if (!articleId) throw new Error(`unresolved article identity for ${article.url}`);
      const attention = await readArticleAttentionPeople(sb, articleId, recentCutoff, attentionEndAt, 12);
      if (attention.availability !== "available" || attention.count == null) throw new Error(`article attention unavailable for ${article.url}`);
      return { article, attention: { ...attention, count: attention.count } };
    });
    for (const { article, attention } of resolvedTopAttention) {
      if (attention.availability !== "available" || attention.count == null) {
        throw new Error(`article attention unavailable for ${article.url}`);
      }
      const knownClinicians = new Set(attention.people.map((person) => normalizeXHandle(person.handle)).filter(Boolean));
      const revealableClinicians = /* @__PURE__ */ new Set();
      for (const post of [...article.posts, ...article.publisherPosts ?? [], ...article.otherPosts ?? []]) {
        const author = normalizeXHandle(post.handle);
        if (author && knownClinicians.has(author)) revealableClinicians.add(author);
        for (const reposter of post.repostedBy ?? []) {
          const handle = normalizeXHandle(reposter.handle);
      ...
      article.kolSharers = attention.count;
      article.faces = attention.people.map((person) => person.avatar).filter(Boolean).slice(0, 5);
      article.revealableClinicianCount = revealableClinicians.size;
    }
    for (const rd of readingByKey.values()) {
    ...
    await pageEachBatch((from, to) => sb.from("v_article_mentions").select("mention_id,article_id,x_post_id,x_source_id,posted_at,source_type,post_kind").eq("source_type", "kol").eq("is_share_kind", true).gte("posted_at", priorCutoff).lt("posted_at", recentCutoff).order("mention_id", { ascending: true }).range(from, to), async (batch) => {
      const evidenceRows = await hydrateMentionEvidence(batch);
      dropBanned(evidenceRows);
      for (const r of evidenceRows) {
        const a = r.x_shared_articles;
    ...
    const fullReuse = !!prev?.proseFp && prev.proseFp === proseFp && !!prev.headline;
    ...
    const recapOut = fullReuse ? { recap: null, headline: prev.headline, headlineStoryId: prev.headlineStoryId ?? null, whys: {}, moverHeadlines: {}, storyWhys: {}, storyHeadlines: {} } : await recapFor(area, topMovers, events, storyCandidates, leadStories);
    let { headline } = recapOut;
    const recap = null;
