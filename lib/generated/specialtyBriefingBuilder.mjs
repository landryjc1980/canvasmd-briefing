  ...
  async function buildArea(area, opts, effects) {
    ...
    await pageEachBatch((from, to) => sb.from("v_article_mentions").select("mention_id,article_id,x_post_id,x_source_id,posted_at,source_type,post_kind").eq("source_type", "kol").eq("is_share_kind", true).gte("posted_at", recentCutoff).order("mention_id", { ascending: true }).range(from, to), async (batch) => {
      ...
      for (const r of evidenceRows) {
        ...
        const articleKey = String(r.article_id ?? "").trim();
        ...
        if (pid) {
          ...
          if (!hit.keys.includes(articleKey)) hit.keys.push(articleKey);
        }
        ...
        if (handle && isCredibleClinician(r.x_sources, clinicianSourceIds) && readHere) {
          ...
          if (!agg) {
            agg = { articleId: articleKey, title: candidateTitle, url: a.canonical_url, journal: a.journal ?? null, domain: a.domain ?? null, doi: a.article_doi ?? null, pmid: a.pmid ?? null, validatedPublisherUrl: a.validated_publisher_url ?? null, publisherUrlStatus: a.publisher_url_status ?? null, publisherUrlSource: a.publisher_url_source ?? null, pubTypes: a.pub_types ?? null, abstract: cleanArticleContext(a.abstract), description: cleanArticleContext(a.description), pubDate: a.pub_date ?? null, firstSharedAt: a.first_shared_at ?? null, areas, sharers: /* @__PURE__ */ new Map(), personSharers: /* @__PURE__ */ new Map(), posts: /* @__PURE__ */ new Map(), topLikes: 0, peerReviewed: isPeerReviewed(a) };
    ...
    await pageEachBatch((from, to) => {
      ...
      for (const r of batch) {
        ...
        for (const rawKey of artHit?.keys ?? []) {
          const agg = articleAgg.get(rawKey);
          ...
          if (agg && safeReaderUrl && !rec.articles.has(rawKey)) rec.articles.set(rawKey, { title: agg.title, url: safeReaderUrl, journal: agg.journal, domain: agg.domain, abstract: agg.abstract, description: agg.description, peerReviewed: agg.peerReviewed });
        }
      }
    });
    ...
    await pageEachBatch((from, to) => sb.from("v_article_mentions").select("mention_id,article_id,x_post_id,x_source_id,posted_at,source_type,post_kind").in("source_type", ["organization", "journal", "news", "brand", "advocacy"]).eq("is_share_kind", true).gte("posted_at", recentCutoff).order("mention_id", { ascending: true }).range(from, to), async (batch) => {
      ...
      for (const r of evidenceRows) {
        ...
        const k = String(r.article_id ?? "").trim();
        if (!k) continue;
        let rd = readingByKey.get(k);
        if (!rd) {
          rd = { articleId: k, title: cleanTitle(a.title), url: a.canonical_url, journal: a.journal ?? null, domain: a.domain ?? null, doi: a.article_doi ?? null, pmid: a.pmid ?? null, validatedPublisherUrl: a.validated_publisher_url ?? null, publisherUrlStatus: a.publisher_url_status ?? null, publisherUrlSource: a.publisher_url_source ?? null, pubTypes: a.pub_types ?? null, abstract: cleanArticleContext(a.abstract), description: cleanArticleContext(a.description), pubDate: a.pub_date ?? null, firstSharedAt: a.first_shared_at ?? null, personSharers: /* @__PURE__ */ new Map(), publishers: /* @__PURE__ */ new Map(), evidence: /* @__PURE__ */ new Map(), topLikes: 0, peerReviewed: isPeerReviewed(a) };
    ...
    const readings = [...readingByKey.values()];
    const attentionEndAt = new Date(now).toISOString();
    const attentionCounts = await readArticleAttention(
      sb,
      readings.map((reading) => reading.articleId),
      recentCutoff,
      attentionEndAt
    );
    const clinicianCount = (reading) => {
      const row = attentionCounts.get(reading.articleId);
      if (!row || row.availability !== "available" || !Number.isInteger(row.clinician_count)) {
        throw new Error(`article attention unavailable for ${reading.articleId}`);
      }
      return Number(row.clinician_count);
    };
    const readingByReaderUrl = /* @__PURE__ */ new Map();
    const readerReadyReadings = readings.filter((a) => isReaderReadySourceTitle(a.title) && a.peerReviewed && !!readerArticleUrl({ ...a, validated_publisher_url: a.validatedPublisherUrl, publisher_url_status: a.publisherUrlStatus, publisher_url_source: a.publisherUrlSource }));
    ...
    const resolvedTopAttention = await mapBounded(topArticles, 4, async (article) => {
      ...
      const attention = await readArticleAttentionPeople(sb, reading.articleId, recentCutoff, attentionEndAt, 12);
    ...
    if (moverIds.length) {
      const stanceRows = await pageAll((from, to) => sb.from("v_pharma_stance_eligible").select("entity_id,source_kind,source_id,stance_valence,practice_signal,stance_axis,occurred_at,evidence,disease_area").in("entity_id", moverIds).gte("occurred_at", stanceCut).order("occurred_at", { ascending: false }).range(from, to));
      const byDrug = /* @__PURE__ */ new Map();
      let smearDropped = 0;
