...
export async function runSpecialtyBatch({ url, key, engineInfo, job, sourceRunId, now = new Date(), signal = undefined, fetchImpl = fetch, record = undefined, onOutput = undefined, allowCapturedRecap = false, mode = "shadow", publicationControl }) {
  ...
  try {
    ...
    requireResult(await receipts.from("readout_specialty_source_runs").insert({ id: job.id, source_run_id: sourceRunId, mode, built_at: builtAt, deadline_at: deadlineAt, engine_sha: engineInfo.backendSha, windows }), "Register specialty batch");
    registered = true;
    ...
    if (mode === "publish") {
      ...
      return await finishSpecialtyPublication(receipts, job.id, diagnostics);
    }
