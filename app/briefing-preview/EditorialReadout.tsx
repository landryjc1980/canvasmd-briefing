...
function PhysicianVoices({ article, accent, sharedBy, expanded }: { article: BriefingArticle | null; accent: string; sharedBy: number; expanded: boolean }) {
  ...
  const visiblePosts = expanded ? previewPosts : previewPosts.slice(0, 1);
  return (
    <div className={`er-voices ${visiblePosts.length === 1 ? "is-single" : ""}`} style={{ "--accent": accent } as React.CSSProperties}>
      ...
      {visiblePosts.map((post, index) => (
        <figure className={`er-voice ${index > 0 ? "er-voice-secondary is-mobile-open" : ""}`} key={`${post.handle ?? post.name}-${index}`}>
...
function CoverageLinks({ item, primaryUrl, expanded }: { item: EditorialArticle; primaryUrl: string; expanded: boolean }) {
  ...
  if (!expanded) {
    const summary = coverageSummary(item, primaryUrl);
    return summary ? <p className="er-also">{summary}</p> : null;
  }
  return (
    <div className="er-support-links">
      ...
      {related.length > 0 && (
        <div className="er-related-links is-open">
          {related.map((link) => (
...
function discloseLabel(item: EditorialArticle, primaryUrl: string, extraComments: number): string {
  const { supportingEvidence, related } = attachedSources(item, primaryUrl);
  const parts: string[] = [];
  if (item.finding.trim()) parts.push(articleContentType(item) === "Regulatory update" ? "Full summary" : "Full abstract");
  if (supportingEvidence.length) parts.push(supportingEvidence.map((link) => link.sourceLabel).join(", "));
  if (related.length) parts.push(`${related.length} related`);
  if (extraComments > 0) parts.push(`Show ${extraComments} more comment`);
...
function ArticleDevelopment({ item, briefs, overlays }: { item: EditorialArticle; briefs: BriefingData[]; overlays: Map<string, BriefingEvidenceOverlayItem> }) {
  ...
  return (
    <article className={`er-development ${open ? "is-open" : ""}`}>
      <div className="er-development-body">
        ...
        <div className="er-proof">
          ...
          {!overlay && <span className="er-proof-count">Updating clinician evidence...</span>}
        </div>
        {overlay && <SharerNames article={article} sharedBy={sharedBy} />}
        {overlay && <PhysicianVoices article={article} accent="currentColor" sharedBy={sharedBy} expanded={open} />}
        {canDisclose && (
          <button className="er-disclose" type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
            <span>{open ? "Show less" : discloseLabel(item, href, extraComments)}</span>
            <span className="er-disclose-chevron" aria-hidden="true" />
          </button>
        )}
      </div>
    </article>
  ...
  return (
    <article className={`er-development er-development-episode ${open ? "is-open" : ""}`}>
      <div className="er-development-body">
        ...
        {canDisclose && (
          <button className="er-disclose" type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
            <span>{open ? "Show less" : "Full description"}</span>
            <span className="er-disclose-chevron" aria-hidden="true" />
          </button>
        )}
      </div>
...
function CompactDevelopment({
  ...
  const [open, setOpen] = useState(false);
  const href = article?.url || item.url;
  const extraComments = 0;
  const canDisclose = Boolean(item.finding.trim()) || Boolean(coverageSummary(item, href));
  return (
    <article className={open ? "is-open" : undefined}>
      ...
      <SourceHeadline href={href} source={item.journal} title={article?.title || item.title} compact />
      <DevelopmentFinding text={item.finding} expanded={open} />
      <CoverageLinks item={item} primaryUrl={href} expanded={open} />
      <div className="er-compact-proof">
        {overlay && <FacePile article={article} count={sharedBy} />}
        {!overlay && <span>Updating clinician evidence...</span>}
      </div>
      {overlay && <SharerNames article={article} sharedBy={sharedBy} />}
      ...
      {pageReady && relevant.length > 0 && (
        <section className="er-section er-relevant">
          ...
          <div className="er-compact-list">{(alsoOpen || relevant.length === 1 ? relevant : relevant.slice(0, 1)).map((item) => {
            ...
            const sharedBy = article?.kolSharers ?? item.sharedBy;
            return (
              <CompactDevelopment key={item.id} item={item} article={article} overlay={overlay} sharedBy={sharedBy} />
            );
          })}</div>
        </section>
      )}
