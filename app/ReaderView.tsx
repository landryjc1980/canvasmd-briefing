...
import { palOf, barSegments, metricsLine, clipTs, heroSplit, heroStats, UP, DOWN } from "./briefVM";
...
function PodCard({ p, accent }: { p: BriefingPod; accent: string }) {
  return (
    <div style={cardBox}>
      ...
      {p.audioUrl
        ? <AudioQuote audioUrl={p.audioUrl} startMs={p.startMs} label={`clip ${clipTs(p.startMs)}`} />
    ...
    <div style={{ minHeight: "100vh", background: pal.bg, color: "#eef1f8", fontFamily: "system-ui,-apple-system,'Segoe UI',sans-serif", transition: "background .45s ease" }}>
      <div style={{ maxWidth: 690, margin: "0 auto", padding: "52px 30px 120px" }}>
        ...
        <div style={{ textAlign: "center", marginTop: 40 }}>
          ...
          {hero.lead && <h1 style={{ font: "400 34px/1.18 'Newsreader',Georgia,serif", color: "#f8f9fc", margin: "16px 0 0", letterSpacing: "-.01em" }}>{hero.lead}</h1>}
          {hero.rest && <p style={{ font: "400 18px/1.6 'Newsreader',Georgia,serif", color: "#b7bac3", margin: "20px auto 0", maxWidth: 560 }}>{hero.rest}</p>}
