...
// evolve this file with the live ReaderView; it deliberately carries its own copy of
// the old StanceBlock so restyles there don't leak in. Also tagged in git:
// design-2026-07-21-flat-reader.
//
...
function StanceBlock({ stance, accent, style }: { stance?: BriefingStance | null; accent: string; style?: React.CSSProperties }) {
...
function PodCard({ p, accent }: { p: BriefingPod; accent: string }) {
  return (
    <div style={cardBox}>
      ...
      {p.audioUrl
        ? <AudioQuote audioUrl={p.audioUrl} startMs={p.startMs} label={`clip ${clipTs(p.startMs)}`} eventId={p.episodeId} eventLabel={p.episodeTitle} accent={accent} tone="dark" />
...
export default function ReaderViewFlat({ data, area, areas, onArea, seen, compact = false }: { data: BriefingData; area: string; areas: string[]; onArea: (a: string) => void; seen?: Record<string, string>; compact?: boolean }) {
          ...
          return (
            ...
            <Row open={openId === id} onToggle={() => toggle(id)} accent={pal.accent}
              ...
              <div style={{ marginLeft: compact ? 0 : 50 }}>
                ...
                {s.papers.length > 0 && <div><div style={evLabel(pal.accent)}>{paperBlockLabel(s)}</div>{s.papers.map((p, j) => <PaperCard key={j} title={p.title} journal={p.journal} domain={p.domain} peerReviewed={p.peerReviewed} meta={paperMeta(p.sharers.length || p.posts?.length || 0, p.sharerCount)} url={p.url} abstract={p.abstract} posts={p.posts?.length ? p.posts : p.sharers} accent={pal.accent} />)}</div>}
              </div>
            </Row>
            </div>
          );
